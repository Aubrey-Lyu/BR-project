%% analysis: raw data, all channel, train HMM
...
if run_HMM
    ...
    result_file = dir(fullfile(output_dir, 'run*.mat'));
    if isempty(result_file)
        run_indx = 0;
    else
        result_name = result_file(end).name;
        run_indx = strsplit(result_name, '.mat');
        run_indx = strsplit(run_indx{1}, 'run');
        run_indx = str2double(run_indx{2});
    end
   
    save(fullfile(output_dir, ['run' num2str(run_indx+1) '.mat']), 'Gamma','vpath','hmm', 'options')
    disp('HMM done.')
    disp('==================================')
    disp(' ')
end
