...
if inv_nii
    ...
    for w = loopin_w
        for i = loopin_i
            ...
            load('/lustre/scratch/wbic-beta/dl577/Binocular_Rivalry/EEG/subs-eeg/newfiles.mat')
%             % define function, filter out certain files with a key word
...
%             Scfiles      = files(mask_files);
            ...
            for ss = loopin_s
                ...
                es_nii = dir(fullfile(subdir, ['edomin*_' sub '_scan*_' num2str(pars.Inv(i)) '_t' twin '*_2.nii']));
                eu_nii = dir(fullfile(subdir, ['eu*_' sub '_scan*_' num2str(pars.Inv(i)) '_t' twin '*_1.nii']));
                es_nii = {es_nii(:).name}';
                eu_nii = {eu_nii(:).name}';
            ...
            select = @(x) x(68:end); % example x(1:67) = '/lustre/scratch/wbic-beta/dl577/Binocular_Rivalry/EEG/subs-eeg/S01/'
            ...
            if isempty(torun_inx);
                ...
                matlabbatch{1}.spm.meeg.source.results.ctype = 'evoked'; % single trials
